    ...
    try {
      ...
      if (files?.length) {
        if(property.propertyPhotos){
          ...
          const oldPropertyPhotos = property.propertyPhotos || [];
          for (const propertyPhoto of oldPropertyPhotos) {
            await this.photoService.deleteImageFromCloudinary(propertyPhoto.publicId);
          }
          await queryRunner.manager.remove(PropertyPhoto, oldPropertyPhotos);
        }
        ...
        for (const file of files) {
          ...
          const propertyPhoto = queryRunner.manager.create(PropertyPhoto, {
            url: uploaded.secure_url,
            publicId : uploaded.public_id,
            property
          });

          const savedPropertyPhoto = await queryRunner.manager.save(PropertyPhoto,propertyPhoto)
          // this.photoService.saveUpload(photoEntity, queryRunner.manager);
          newPhotos.push(savedPropertyPhoto);
        }

        property.propertyPhotos = newPhotos;
      }
